...
class Puzzle:
    ...
    def solve(self) -> int:
        ...
        for number in self.numbers:
            ...
            if len(self.unsolved_boards) == 1:
                last_board: BingoBoard = self.unsolved_boards[0]

            if len(self.solved_boards) == len(self.boards):
                break
    ...
    @property
    def unsolved_boards(self):
        unsolved_boars: list[BingoBoard] = []
        for board in self.boards:
            if not board.solved:
                unsolved_boars.append(board)
        return unsolved_boars
